...
def _market_state(obs, step):
    seat = _seat(obs)
    state = _MARKET_STATE[seat]
    if step == 0 or step < int(state.get("last_step", -1)):
        state = {"last_step": step, "deferred": {}, "front_debt": {}}
        _MARKET_STATE[seat] = state
    state["last_step"] = step
    return state


def _demand_units(obs, item, step):
...
def _next_demand_release(obs, item, step):
    for tick in range(step, step + _DEFER_MAX_STEPS):
        if _demand_units(obs, item, tick) > 0:
            return tick + 1
    return None
...
def _live_sale_capacity(action, obs, item):
    private = _get(obs, "private", {}) or {}
    shed = _get(private, "shed", {}) or {}
    stock = _positive_int(_get(shed, item, 0) or 0)
    return max(0, stock - _pickup_reserve(action, item))


def _add_sale(action, item, quantity):
...
def _should_defer_sale(obs, item, step):
    if item not in _PREMIUM_BASE_PRICES or step >= _FINAL_FLUSH_STEP:
        return None
    private = _get(obs, "private", {}) or {}
    shed = _get(private, "shed", {}) or {}
    if sum(_positive_int(value) for value in dict(shed).values()) > _DEFER_SHED_GUARD:
        return None
    market = _get(obs, "market", {}) or {}
    prices = _get(market, "prices", {}) or {}
    if _positive_int(_get(prices, item, _PREMIUM_BASE_PRICES[item])) >= _PREMIUM_BASE_PRICES[item]:
        return None
    release_step = _next_demand_release(obs, item, step)
    if release_step is None or release_step > step + _DEFER_MAX_STEPS:
        return None
    return release_step


def _apply_demand_deferral(action, obs, state, step):
    action = _copy_action(action)
    original_market = list(action.get("market", []))
    action["market"] = []

    for raw in original_market:
        order = list(raw)
        if len(order) < 3 or order[0] != "SELL" or order[1] not in _PREMIUM_BASE_PRICES:
        ...
        capacity = max(
            0,
            _live_sale_capacity(action, obs, item) - _existing_sell(action, item),
        )
        quantity = min(_positive_int(order[2]), capacity)
        release_step = _should_defer_sale(obs, item, step)
        if release_step is None:
            _add_sale(action, item, quantity)
            continue
        pending = state.setdefault("deferred", {}).get(item)
        already_pending = _positive_int(pending.get("quantity", 0)) if pending else 0
        quantity = min(quantity, max(0, capacity - already_pending))
        if quantity <= 0:
            continue
        if pending is None:
            state["deferred"][item] = {
                ...
                "deadline": min(step + _DEFER_MAX_STEPS, release_step),
            }
        else:
            pending["quantity"] = already_pending + quantity
            pending["release_step"] = min(int(pending["release_step"]), release_step)
            pending["deadline"] = min(int(pending["deadline"]), step + _DEFER_MAX_STEPS)

    for item, pending in list(state.setdefault("deferred", {}).items()):
        due = (
            step >= int(pending.get("release_step", step))
            or step >= int(pending.get("deadline", step))
            or step >= _FINAL_FLUSH_STEP
        )
        if not due:
            continue
        capacity = max(
        ...
        quantity = min(_positive_int(pending.get("quantity", 0)), capacity)
        if quantity > 0 and not _add_sale(action, item, quantity):
            continue
        state["deferred"].pop(item, None)

    action["market"] = action.get("market", [])[:10]
    return action


def _repay_front_debt(action, state, step):
    due = dict(state.setdefault("front_debt", {}).pop(step, {}))
    action = _copy_action(action)
    if not due:
        return action
    market = []
    for raw in action.get("market", []):
        order = list(raw)
        if len(order) >= 3 and order[0] == "SELL" and order[1] in due:
            item = order[1]
            quantity = _positive_int(order[2])
            reduction = min(quantity, _positive_int(due[item]))
            quantity -= reduction
            due[item] = _positive_int(due[item]) - reduction
            if quantity <= 0:
                continue
            order[2] = quantity
        market.append(order)
...
def _scheduled_sell_quantity(step, item):
    if not 0 <= step < len(_ACTIONS):
        return 0
    return sum(
        _positive_int(order[2])
        for order in (_ACTIONS[step].get("market") or [])
        if len(order) >= 3 and order[0] == "SELL" and order[1] == item
    )


def _adaptive_front_run(action, obs, state, step):
    action = _copy_action(action)
    debts = state.setdefault("front_debt", {})
    for item in _PREMIUM_BASE_PRICES:
        capacity = max(
            0,
            _live_sale_capacity(action, obs, item) - _existing_sell(action, item),
        )
        if capacity <= 0:
            continue
        for due_step in range(step + 1, min(len(_ACTIONS), step + 5)):
            if _demand_units(obs, item, due_step - 1) > 0:
                break
            scheduled = _scheduled_sell_quantity(due_step, item)
            if scheduled <= 0:
                continue
            due = debts.setdefault(due_step, {})
            remaining = max(0, scheduled - _positive_int(due.get(item, 0)))
            quantity = min(remaining, capacity)
            if quantity > 0 and _add_sale(action, item, quantity):
                due[item] = _positive_int(due.get(item, 0)) + quantity
            if not due:
                debts.pop(due_step, None)
            break
    action["market"] = action.get("market", [])[:10]
    return action
